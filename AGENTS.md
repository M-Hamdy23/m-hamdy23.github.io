...
- Current redesign page (`index.html`) is self-contained (inline `<style>` + `<script>`) and data-driven:
  - `PORTOFOLIO_DATA` is the single source of truth for rendered sections (`HERO_DATA`, `SKILLS_DATA`, `EXPERIENCE_DATA`, `EDUCATION_DATA`, `CONTACT_DATA`, `FOOTER_DATA`) and nested project data in `PORTOFOLIO_DATA.PROJECTS`.
...
- For quick validation, open the relevant page directly in a browser and verify:
  - In `index.html`, sections render from `PORTOFOLIO_DATA` (hero/skills/experience/education/contact/footer), project filter buttons repopulate `#projects-grid`, clicking a project card opens/closes the shared overlay modal, and resume/social links resolve.
...
- In redesign pages, add/edit project data only through the data arrays (`PORTOFOLIO_DATA.PROJECTS` in `index.html`, `PROJECTS` in `index_New.html`); avoid hardcoding project cards in markup.
- In redesign pages, keep filter button arguments like `filterProjects('pc', this)` aligned with lowercase values in project `tags`.
- In `index.html`, keep `PORTOFOLIO_DATA.PROJECTS[].id` unique numeric values because card clicks call `openModal(id)` directly.
